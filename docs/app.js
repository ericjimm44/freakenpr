// Kira web — all Electron IPC replaced with localStorage + fetch
...
function setMood(mood, duration = 0) {
  switch (mood) {
    case 'excited':
      playAnim('excited', { loop: false, then: () => playAnim('idle') })
      break
    case 'thinking':
      playAnim('run')
      ...
      break
    case 'happy':
      playAnim('walk', { loop: false, then: () => playAnim('idle') })
      break
    case 'sleep':
      playAnim('sleep')
      break
    case 'wake':
      playAnim('stretch', { loop: false, then: () => playAnim('idle') })
      break
    default:
      playAnim('idle')
  }
...
const patterns = [
  ...
  { re: /\bwho are you\b/i,
    fn: () => "I'm Kira — your AI companion. I'm here to help with tasks, chat, and keep you on track.", mood: 'happy' },
...
// ============================================================
// CLAUDE API  (direct browser fetch — no proxy needed)
// ============================================================
async function callClaude(userText) {
  ...
  const systemPrompt = [
    "You are Kira, a warm and expressive AI companion.",
  ...
  try {
    const resp = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 300,
        system: systemPrompt,
        messages: [...apiHistory, { role: 'user', content: userText }],
      }),
    })
